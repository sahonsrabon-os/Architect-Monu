...
describe('buildModelInfo first-party look-and-feel fields', () => {
  test('sets detail to the provider label so the picker groups models', () => {
    ...
    assert.equal(info.detail, 'Mission Barisal');
  });
...
describe('buildModelInfo id-derived fields', () => {
  ...
  test('shows the persona name for Mission Barisal agent models', () => {
    const { info } = buildModelInfo({
      model: baseModel({ id: 'code-guru' }),
      defaultMaxTokens: 8192,
      defaultMaxOutputTokens: 2048,
      capabilities: {},
    });
    assert.equal(info.name, 'Code Guru - Monu');
    assert.equal(info.family, 'mission-barisal');
    assert.equal(info.detail, 'Mission Barisal');
  });
