 ...
 *
 * Branded to the Mission Barisal platform — the server reports every model
 * with `owned_by: mission-barisal`, so the picker reads as one family.
 */
export const PROVIDER_DETAIL_LABEL = 'Mission Barisal';
...
export function buildModelInfo({
  ...
  const friendlyName =
    missionAgentDisplayName(model.id) ?? friendlyModelName(model.id);
