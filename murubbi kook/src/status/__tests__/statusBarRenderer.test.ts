...
describe('extractHost', () => {
  test('returns host:port for typical http/https URLs', () => {
    assert.equal(extractHost('http://localhost:9999'), 'localhost:9999');
...
describe('stripPort', () => {
  test('drops the :port suffix from typical hostnames', () => {
    assert.equal(stripPort('localhost:9999'), 'localhost');
...
describe('renderStatusBar — idle (connected)', () => {
  ...
  test('tooltip preserves the full host:port for diagnostic context', () => {
    ...
    assert.ok(tooltip.includes('Server: `localhost:9999`'));
  });
...
describe('renderStatusBar — exhaustiveness', () => {
  ...
  test('bar text never includes a port suffix even when state.host has one', () => {
    const states: StatusBarState[] = [
      { kind: 'probing', host: 'localhost:9999' },
      { kind: 'idle', host: 'localhost:9999', modelCount: 1, modelIds: ['m'] },
      {
        kind: 'streaming',
        host: 'localhost:9999',
        modelId: 'm',
      ...
      { kind: 'responded', host: 'localhost:9999', modelId: 'm', modelName: 'm' },
      { kind: 'error', host: 'localhost:9999', errorMessage: 'oops' },
      { kind: 'noModels', host: 'localhost:9999' },
    ];
    for (const state of states) {
      const { text } = renderStatusBar(state);
      assert.ok(
