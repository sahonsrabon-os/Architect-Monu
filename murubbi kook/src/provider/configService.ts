...
export class ConfigService {
    ...
    if (this.lastInvalidUrlNotified !== url) {
      ...
      setImmediate(() => {
        this.deps.promptOpenSettings(
          `ZombieCoder Mission Barisal: Invalid Server URL ${JSON.stringify(url)}. Open Settings to fix.`
        );
      });
    }
  }
  ...
  private reportOutputTokensAdjusted(
    ...
    this.deps.log(
      `WARNING: zombiecoder.mission-barisal.defaultMaxOutputTokens (${issue.output}) >= defaultMaxTokens (${issue.total}). Adjusting to ${issue.adjusted}.`
    );
    ...
    if (last?.output !== issue.output || last?.total !== issue.total) {
      ...
      vscode.window.showWarningMessage(
        `ZombieCoder Mission Barisal: 'defaultMaxOutputTokens' was >= 'defaultMaxTokens'. Adjusted to ${issue.adjusted} to avoid request errors.`
      );
    }
  }
}
