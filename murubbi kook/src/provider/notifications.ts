...
function promptToolCallingError(log: Logger, showOutput: () => void): void {
  vscode.window
    .showErrorMessage(
      `ZombieCoder Mission Barisal: Model failed to generate valid tool calls. This model may not support function calling. Check Output panel for details.`,
      'Open Output',
    ...
    .then(
      (selection: string | undefined) => {
        if (selection === 'Open Output') {
          ...
          vscode.workspace
            .getConfiguration('zombiecoder.mission-barisal')
...
export function handleChatError(error: unknown, log: Logger, showOutput: () => void): never {
  ...
  if (overflowContext !== undefined) {
    ...
    promptOpenSettings(
      `ZombieCoder Mission Barisal: Request exceeded the model's context window (server reports ${overflowContext} tokens). ` +
    ...
    vscode.window.showErrorMessage(
      `ZombieCoder Mission Barisal: Chat request failed. ${errorMessage}`
    );
  }
