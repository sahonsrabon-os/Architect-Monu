...
import { isValidServerUrl } from '../provider/configValidation';
...
export async function configureServerFlow(
  ...
  const currentUrl = config.get<string>('serverUrl', 'http://localhost:9999');
  ...
  const url = await vscode.window.showInputBox({
    ...
    validateInput: (value) => {
      ...
      return 'Please enter a valid URL or Unix socket path (e.g. /tmp/zombiecoder/mcp.sock)';
    },
  });
