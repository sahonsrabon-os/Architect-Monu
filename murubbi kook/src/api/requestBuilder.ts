...
export interface ChatRequestOptions {
  ...
  /**
   * Per-workspace session id — the server scopes session memory + syllabus
   * learning to this id (per-project isolation across many folders).
   */
  sessionId?: string;
  /**
   * Workspace folder path — forwarded so the server auto-generates and reads
   * `.zombiecoder/SSOT.md` + `syllabus.md` inside THIS project folder.
   */
  workspace?: string;
}
