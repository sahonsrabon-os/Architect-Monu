...
export function convertMessage(
    ...
    switch (part.kind) {
      ...
      case 'image':
        if (!options.enableImageInput) {
          log(
            `  Skipping data part: mimeType=${part.mimeType}, size=${part.data.length} bytes. (Please enable zombiecoder.mission-barisal.enableImageInput in settings)`
          );
          break;
        }
  ...
  if (toolCalls.length > 0) {
    ...
    // Tool results are emitted first (role: 'tool').  If the same message
    // also carries user text (e.g. VS Code bundles previous tool results
    // with the user's follow-up), append the text as a separate user
    // message so the model actually sees the input — previously the text
    // was silently dropped (issue: "model receives no input text").
    result.push(...toolResults);
    if (textContent) {
      result.push({ role: message.role, content: textContent });
    }
  } else if (hasImages) {
    // Multimodal: keep the rich content array (text + image_url parts).
    result.push({ role: message.role, content: userContent });
  } else if (textContent) {
    // Pure-text messages collapse into the plain string form. The Mission
    // Barisal server contract requires `content` to be a string; sending the
    // array form made the server reply 500 / blank for stream=false (and
    // some UDS handlers), which the user saw as an empty chat message.
    result.push({ role: message.role, content: textContent });
  }
  return result;
}
